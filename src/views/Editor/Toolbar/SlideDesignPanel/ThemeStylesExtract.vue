<template>
  <div class="theme-styles-extract">
        ...
        <div class="values">
          <div class="value-wrap" v-for="(item, index) in themeStyles.fontNames" :key="item">
            ...
            <div class="handler">
              ...
              <div class="config-btn" @click="updateTheme({ fontName: item }); selectedIndex.fontName = index">Apply</div>
            </div>
          </div>
        </div>
      </div>
        ...
        <div class="values">
          <div class="value-wrap" v-for="(item, index) in themeStyles.fontColors" :key="item">
            ...
            <div class="handler">
              ...
              <div class="config-btn" @click="updateTheme({ fontColor: item }); selectedIndex.fontColor = index">Apply</div>
            </div>
          </div>
        </div>
      </div>
        ...
        <div class="values">
          <div class="value-wrap" v-for="(item, index) in themeStyles.backgroundColors" :key="item">
            ...
            <div class="handler">
              ...
              <div class="config-btn" @click="updateTheme({ backgroundColor: item }); selectedIndex.backgroundColor = index">Apply</div>
            </div>
          </div>
        </div>
      </div>
      <div class="config-item" v-if="themeStyles.themeColors.length">
        <div class="label">Theme Colors: <span class="tip">（Click the color block to exclude unwanted colors）</span></div>
    ...
    <div class="btns">
      <Button class="btn" type="primary" @click="updateAllThemes()"><IconCheck /> Save Selected Configurations as Theme</Button>
    </div>
  </div>
</template>
...
const updateAllThemes = () => {
  ...
  if (themeColors.length > 6) {
    ...
    message.warning('Theme colors exceed the limit, automatically selecting the first 6.')
  }
