...
// Current selection border line position
...
// Current selection resize point position
...
// Keyboard shortcut listener: Move focus to the next row on Enter
const moveNextRow = () => {
...
// Get current chart data from the DOM and send it out formatted
const getTableData = () => {
  ...
  // First row is series names, first column is item names, actual data starts from the second row and column
  ...
  // Handle special chart types that need custom data processing
  // Scatter chart only has two columns of data
...
// Custom paste event (attempts to read table data from the clipboard)
...
// Mouse drag to modify selected range
const changeSelectRange = (e: MouseEvent) => {
  ...
  document.onmouseup = e => {
    ...
    // When dragging ends, if the range exceeds half a cell, automatically expand to the next cell
